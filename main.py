...
# 2. 數據引擎：暴力鎖定 202603 且過濾成交量最大的「全盤」
@st.cache_data(ttl=10)
def fetch_target_2603():
    try:
        dl = DataLoader()
        ...
        if df is not None and not df.empty:
            ...
            # 【關鍵】：同一天會有兩筆 202603，取成交量大的那一筆 (就是全盤 32,013)
...
if df is not None and len(df) >= 1:
    ...
    st.warning("📊 正在鎖定 微台03全 數據... 請點擊 Reboot 重啟環境。")
